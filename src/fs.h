...
static inline int mkdir_p(std::string dir, uid_t user, gid_t group, mode_t mode)
{
...
static inline int mkdir_p(std::string dir)
{
  if (mkdir(dir.c_str(), S_IRWXG | S_IRWXO | S_IRWXU) < 0) {
    if (errno == ENOENT) {
      size_t slash = dir.rfind('/');
...
static inline int rmdir_p(std::string directory)
{
  assert( (directory != "") );
